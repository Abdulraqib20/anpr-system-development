    ...
    def auto_detection_loop(self, camera_cap):
        ...
        while auto_detection_running and camera_cap:
            try:
                ...
                # Check database pool health and adjust delays accordingly
                try:
                    if hasattr(builtins, 'log_pool_status'):
                        # Test pool health without logging unless there's an issue
                        test_conn = db_pool.getconn()
                        db_pool.putconn(test_conn)
                        # Pool is healthy, use normal delays
                        if vehicles:
                            time.sleep(0.3)  # Faster checking when vehicles detected
                        else:
                            time.sleep(0.7)  # Slower when no vehicles
                except Exception:
                    # Pool is under stress, reduce processing frequency
                    logger.warning("Database pool under stress, reducing auto-detection frequency")
                    if vehicles:
                        time.sleep(0.8)  # Slower even with vehicles
                    else:
                        time.sleep(1.5)  # Much slower when no vehicles
...
# Configure for Unicode support on Windows
import sys
import codecs
if sys.platform == "win32":
    ...
    try:
        ...
        # Fallback: use a stream handler that replaces unsupported characters
        pass

stream_handler = logging.StreamHandler()
stream_handler.setFormatter(log_formatter)
# Configure stream handler to handle Unicode safely
if hasattr(stream_handler.stream, 'encoding') and stream_handler.stream.encoding:
    # Already has encoding support
    logger.addHandler(stream_handler)
else:
    # Create a safer handler that replaces problematic characters
    ...
    safe_handler.setFormatter(log_formatter)
...
try:
    db_pool = SimpleConnectionPool(
        minconn=2,
        maxconn=15, # Increased from 5 to handle concurrent operations
        host=DB_HOST,
        dbname=DB_NAME,
        user=DB_USER,
    ...
    logger.info("Database connection pool created successfully with 15 max connections.")

    # Add pool monitoring function
    def log_pool_status():
        """Log current database pool status for monitoring."""
        if db_pool:
            # Note: psycopg2.pool doesn't have direct methods to check pool status,
            # but we can infer from successful connection attempts
            try:
                test_conn = db_pool.getconn()
                db_pool.putconn(test_conn)
                logger.debug("Database pool health check: OK")
            except Exception as e:
                logger.warning(f"Database pool health check failed: {e}")

    # Store pool monitoring function globally for use in other parts of the app
    import builtins
    builtins.log_pool_status = log_pool_status
except Exception as e:
            ...
            try:
                ...
                if "connection pool exhausted" in str(e).lower():
                    if attempt < max_retries - 1:
                        ...
                        logger.error(f"Connection pool exhausted after {max_retries} attempts. Consider increasing pool size or optimizing connection usage.")
